...
- `CollectionImporter` is destructive: soft-deletes all existing `user_stickers` (all states including `incoming`), then bulk-inserts new rows with `state: glued` and `state: duplicate` from the parsed data. Trades are never touched — they survive reimport.
